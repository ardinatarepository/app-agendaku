...
import {
  ...
  ScrollView, RefreshControl, Alert, Modal, StyleSheet, Platform, StatusBar,
} from 'react-native';
...
function TaskFormModal({ visible, task, onClose, onSubmit, isLoading, categories }) {
  ...
  const handleSubmit = () => {
    ...
    const finalDeadline = form.deadline 
      ? new Date(`${form.deadline}T${form.time}:00`).toISOString() 
      : null;
...
export default function TaskListScreen({ route }) {
  ...
  const createMut = useMutation({
    ...
    onSuccess: async (res) => {
      ...
      setToast({ visible: true, message: 'Tugas berhasil ditambahkan!', type: 'success' });
      
      if (res.data?.data) scheduleTaskNotification(res.data.data, true);
    },
  ...
  const updateMut = useMutation({
    ...
    onSuccess: async (res, variables) => {
      ...
      setShowForm(false);
      setEditTask(null);
      setToast({ visible: true, message: 'Tugas berhasil diperbarui!', type: 'success' });
      ...
      <View style={{ height: 50, marginBottom: 4 }}>
        <FlatList
          ...
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity 
                onPress={() => setFilters(f => ({ ...f, status: item.id }))}
