...
const TaskCard = ({
  ...
  const priorityColors = {
    TINGGI: 'bg-[#FEF2F2] border-red-200',
    NORMAL: 'bg-[#FFFBEB] border-amber-200',
    RENDAH: 'bg-[#F8FAFC] border-slate-200',
  }[task.priority || 'NORMAL'] || 'bg-white border-slate-100';

  return (
    <div
      className={`bg-white rounded-[20px] border shadow-sm overflow-hidden flex flex-col min-h-[120px] transition-all group cursor-pointer
        ${(isFinished || isOverdue) ? 'opacity-60' : ''}
        ${isOverdue && !isFinished ? 'bg-red-50/30' : ''}
        ${isSelected ? 'border-[#FACC15] border-2 shadow-[0_0_0_3px_rgba(250,204,21,0.15)]' : 'border-slate-100'}
      `}
    ...
    >
      ...
      <div className="p-4 flex-1">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-1.5 text-slate-400">
            ...
            <span className={`text-[10px] font-normal tracking-tight ${isOverdue && !isFinished ? 'text-red-500' : ''}`}>
          ...
          <div className="relative">
            <div className="flex items-center gap-2">
              {isFinished && (
                <button
                  onClick={(e) => { e.stopPropagation(); onStatusChange(task.id, 'SEDANG_DIKERJAKAN'); }}
        ...
        <div className="flex items-center gap-2 mt-3">
          <div
            className="px-2.5 py-1 rounded-md text-[9px] font-normal uppercase tracking-widest"
          ...
          {task.category && (
            <div
              className="w-[22px] h-[22px] rounded-full flex items-center justify-center text-[9px] font-black text-white border-2 border-black"
          ...
          {totalSub > 0 && (
            ...
            >
              ...
              <span className="text-[10px] font-normal text-[#334155]">{doneSub}/{totalSub}</span>
          ...
          {totalSub === 0 && !isFinished && (
            <button
              onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }}
              className="ml-auto text-[10px] font-normal text-slate-300 hover:text-black transition-colors uppercase tracking-widest"
            >
                ...
                <div key={st.id} className="flex items-center justify-between group/st">
                  ...
                  >
                    {st.isDone ? (
                      ...
                      <IoEllipseOutline size={24} className="text-[#D1D5DB]" />
                    )}
                    <span className={`text-[13px] font-normal ${st.isDone ? 'line-through text-[#94A3B8]' : 'text-slate-600'}`}>
                      {st.title}
                    </span>
                  </button>
                  <button
                    onClick={(e) => { e.stopPropagation(); onDeleteSubtask(task.id, st.id); }}
              ...
              {!isFinished && (
                <div className="flex items-center gap-3 pt-1">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent border-none text-[14px] font-bold text-slate-600 outline-none placeholder:text-slate-300 placeholder:font-medium"
                    value={newSub}
      ...
      <div className="bg-black p-3 px-5 flex items-center justify-between">
        ...
        {!(isFinished || isOverdue) && (
          <button
            onClick={(e) => { e.stopPropagation(); onStatusChange(task.id, 'SELESAI'); }}
