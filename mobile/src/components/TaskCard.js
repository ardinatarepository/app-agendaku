...
const TaskCard = ({ task, onPress, onEdit, onDelete, onStatusChange, onSubtaskToggle, onAddSubtask, readonly = false }) => {
  ...
  const openMenu = () => {
    ...
    // requestAnimationFrame to avoid conflict with re-renders
    requestAnimationFrame(() => {
      moreBtnRef.current.measure((x, y, width, height, pageX, pageY) => {
        ...
        // Fallback positioning if measure fails on some Android devices
        const topPos = (pageY !== undefined) ? (pageY + height + 5 - statusBarH) : 200;
        const rightPos = (pageX !== undefined) ? (Dimensions.get('window').width - (pageX + width)) : 20;
        
        setMenuPos({ top: topPos, right: rightPos });
        setShowActions(true);
      });
    });
  };
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: fadeAnim.interpolate({ inputRange: [0, 1], outputRange: [20, 0] }) }] }}>
      <View style={[styles.card, isFinished && styles.cardFinished]}>
        {/* Transparent Pressable for the whole card background */}
        <Pressable 
          onPress={onPress}
          style={({ pressed }) => [StyleSheet.absoluteFill, pressed && { backgroundColor: 'rgba(255,255,255,0.03)' }]}
        />
        
        {/* Content Container (pointerEvents="box-none" so buttons inside can be clicked) */}
        <View pointerEvents="box-none">
          ...
          <View style={styles.titleContainer} pointerEvents="box-none">
            ...
            <Text style={[styles.title, isFinished && styles.titleDone]} pointerEvents="none" numberOfLines={2}>
              {task.title}
            </Text>
          </View>

          {/* Badges */}
          <View style={styles.badgesRow} pointerEvents="box-none">
...
const styles = StyleSheet.create({
  card: { backgroundColor: '#0F172A', borderRadius: 20, padding: 18, marginBottom: 12, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)', ...SHADOW.md, position: 'relative', overflow: 'hidden' },
  cardFinished: { opacity: 0.7 },
  headerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, zIndex: 10 },
  ...
  titleContainer: { flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 16, zIndex: 10 },
  checkbox: { padding: 2 },
  title: { flex: 1, fontSize: 17, ...FONT.bold, color: '#f8fafc', lineHeight: 24 },
  titleDone: { textDecorationLine: 'line-through', color: '#475569' },
  badgesRow: { flexDirection: 'row', gap: 10, alignItems: 'center', marginBottom: 20, zIndex: 10 },
  ...
  footer: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingTop: 12, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.04)', zIndex: 10 },
