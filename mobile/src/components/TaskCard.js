...
export default function TaskCard({ task, onPress, onEdit, onDelete, onStatusChange, onSubtaskToggle, onAddSubtask, readonly = false }) {
  ...
  const openMenu = () => {
    moreBtnRef.current?.measure((x, y, width, height, pageX, pageY) => {
      ...
      setMenuPos({ top: pageY + height + 4 - sbH, right: Dimensions.get('window').width - (pageX + width) });
      setShowActions(true);
    });
  };
  ...
  const renderLeftActions = () => {
    ...
    return (
      <TouchableOpacity onPress={() => { swipeRef.current?.close(); onStatusChange(task.id, 'SELESAI'); }} style={[styles.swipeAction, { backgroundColor: '#10b981' }]}>
  ...
  const renderRightActions = () => {
    ...
    return (
      <TouchableOpacity onPress={() => { swipeRef.current?.close(); onDelete(task.id); }} style={[styles.swipeAction, { backgroundColor: '#ef4444' }]}>
      ...
      <Swipeable ref={swipeRef} renderLeftActions={renderLeftActions} renderRightActions={renderRightActions} friction={2}>
        <View style={[styles.card, isFinished && styles.cardFinished]}>
          ...
          <View style={styles.headerRow}>
            ...
            {!readonly && (
              <TouchableOpacity ref={moreBtnRef} onPress={openMenu} style={styles.menuTrigger}>
          ...
          <View style={styles.titleRow}>
            {!readonly && (
              <TouchableOpacity onPress={() => onStatusChange(task.id, isFinished ? 'SEDANG_DIKERJAKAN' : 'SELESAI')}>
                <Ionicons name={isFinished ? 'checkmark-circle' : 'ellipse-outline'} size={26} color={isFinished ? '#10b981' : '#475569'} />
              </TouchableOpacity>
            )}
            <TouchableOpacity onPress={onPress} style={{ flex: 1 }}>
              <Text style={[styles.title, isFinished && styles.titleDone]} numberOfLines={2}>{task.title}</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.footer}>
            <View style={styles.footerLeft}>
              <TouchableOpacity onPress={totalSub > 0 ? toggleExpand : undefined} style={styles.footerItem}>
                <Feather name="layers" size={14} color={expanded ? '#3b82f6' : '#64748b'} />
                <Text style={[styles.footerText, expanded && { color: '#3b82f6' }]}>{totalSub}</Text>
                {totalSub > 0 && <Feather name={expanded ? 'chevron-up' : 'chevron-down'} size={12} color={expanded ? '#3b82f6' : '#64748b'} />}
              </TouchableOpacity>
            ...
            <View style={styles.footerItem}>
              ...
              <Text style={[styles.footerText, { ...FONT.bold }, overdue && { color: '#f87171' }]}>{getTimeRemaining() || '--'}</Text>
            </View>
          </View>
          ...
          {expanded && totalSub > 0 && (
            <View style={styles.subtaskList}>
              {subtasks.map(st => (
                <TouchableOpacity key={st.id} style={styles.subtaskRow} onPress={() => !readonly && onSubtaskToggle(task.id, st.id)}>
                  <Ionicons name={st.isDone ? 'checkmark-circle' : 'ellipse-outline'} size={18} color={st.isDone ? '#10b981' : '#475569'} />
      ...
      <Modal visible={showActions} transparent animationType="fade" onRequestClose={() => setShowActions(false)}>
        <Pressable style={styles.overlay} onPress={() => setShowActions(false)}>
          <View style={[styles.menuCard, { top: menuPos.top, right: menuPos.right }]}>
            <Pressable style={styles.menuRow} onPress={() => { setShowActions(false); onEdit(task); }}>
              <Feather name="edit-2" size={16} color="#e2e8f0" />
              <Text style={styles.menuLabel}>Edit Tugas</Text>
            </Pressable>
            <View style={styles.menuSep} />
            <Pressable style={styles.menuRow} onPress={() => { setShowActions(false); onDelete(task.id); }}>
...
const styles = StyleSheet.create({
  card: { backgroundColor: '#111827', borderRadius: 20, padding: 20, marginBottom: 14, borderWidth: 1, borderColor: '#1e293b' },
  ...
  categoryPill: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#1e293b', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 99, gap: 7 },
  ...
  subtaskList: { marginTop: 16, paddingTop: 14, borderTopWidth: 1, borderTopColor: '#1e293b' },
  ...
  swipeAction: { justifyContent: 'center', alignItems: 'center', width: 80, borderRadius: 20, marginBottom: 14 },
  swipeLabel: { color: '#fff', fontSize: 11, ...FONT.bold, marginTop: 4 },
  overlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' },
  menuCard: { position: 'absolute', backgroundColor: '#1e293b', borderRadius: 14, padding: 6, minWidth: 180, borderWidth: 1, borderColor: '#334155', ...SHADOW.md },
  menuRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, paddingHorizontal: 14, borderRadius: 8 },
